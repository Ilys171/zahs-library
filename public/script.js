...
function getAccessText(days) {
  if (currentLang === "az") return `${days} günlük ${t("accessWord")}`;
  return `${days} ${getDayWord(days)} ${t("accessWord")}`;
}

function getDaysLeftText(days) {
  if (currentLang === "az") return `${days} gün qalıb`;
  if (currentLang === "ru") return `Осталось ${days} ${getDayWord(days)}`;
  return `${days} ${getDayWord(days)} left`;
}

function getBorrowText(days) {
  if (currentLang === "az") return `${days} gün ${t("borrowSuffix")}`;
...
function showView(viewId) {
  views.forEach((view) => view.classList.toggle("visible", view.id === viewId));
...
function updateLoginButton() {
  ...
  if (state.user) {
    ...
    if (loginHeroBtn) loginHeroBtn.textContent = t("goCatalog");
  } else {
    ...
    if (loginHeroBtn) loginHeroBtn.textContent = t("studentLogin");
  }
}
...
function renderCatalog() {
  ...
  list.forEach((book) => grid.appendChild(bookCard(book)));
  observeRevealElements();
}
...
async function loginUser() {
  ...
  if (!/^\d{4}$/.test(code)) return alert(t("loginCodeAlert"));
  if (!password) return alert(t("passwordAlert"));
  try {
...
async function borrowBook(bookId) {
  ...
  if (activeLoans.length > 0) return alert(t("activeLoanAlert"));
  try {
...
function showBookInfo(bookId) {
  ...
  infoContent.innerHTML = `
    ...
    <div class="notice">${getAccessText(getLoanDays(book))}<br>${book.pages} ${t("pagesShort")}</div>
...
function openReader(bookId) {
  if (!state.user) return openLogin();
  if (!isLoanActive(bookId)) return alert(t("needBorrowAlert"));
  const book = books.find((item) => Number(item.id) === Number(bookId));
  if (!book) return alert(t("bookNotFoundAlert"));
  const readerWindow = window.open("", "_blank");
  if (!readerWindow) return alert(t("popupAlert"));
    ...
    <head>
      ...
      <style>
        * { box-sizing: border-box; }
        ...
        .topbar { min-height: 58px; background: #5b5d91; color: white; display: flex; justify-content: space-between; align-items: center; gap: 18px; padding: 14px 22px; }
        .topbar span { color: #e8ddbd; font-size: 14px; }
        iframe { width: 100%; height: calc(100vh - 58px); border: none; background: white; }
        .watermark { position: fixed; right: 18px; bottom: 18px; color: rgba(91, 93, 145, 0.65); background: rgba(255, 255, 255, 0.85); border: 1px solid rgba(91, 93, 145, 0.18); padding: 8px 12px; border-radius: 999px; font-size: 13px; }
      </style>
    </head>
    <body>
      <div class="topbar">
        <strong>${escapeHtml(book.title)}</strong>
        <span>${getDaysLeftText(remaining)} · ${escapeHtml(state.user.studentCode)}</span>
      </div>
      <iframe src="${readUrl}"></iframe>
      <div class="watermark">ZAHS Library · Student ${escapeHtml(state.user.studentCode)}</div>
    </body>
    </html>
  `);
  ...
  revealObserver = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) entry.target.classList.add("visible-reveal");
      });
    },
    { threshold: 0.12 },
  );
...
async function submitFeedback(event) {
  ...
  if (!category)
    return showStatus(feedbackStatus, t("feedbackCategoryError"), "error");
  if (!rating)
    return showStatus(feedbackStatus, t("feedbackRatingError"), "error");
  if (message.length < 5)
    return showStatus(feedbackStatus, t("feedbackMessageError"), "error");
  try {
...
async function submitBookRequest(event) {
  ...
  if (title.length < 2)
    return showStatus(requestStatus, t("requestTitleError"), "error");
  try {
...
async function loadPublicFeedbacks() {
  ...
  feedbacksList.innerHTML = `<div class="empty-state">${t("feedbacksLoading")}</div>`;
  try {
    const feedbacks = await apiRequest("/api/feedbacks");
    if (!feedbacks.length) {
      feedbacksList.innerHTML = `<div class="empty-state">${t("feedbacksEmpty")}</div>`;
      return;
    }
    ...
    feedbacks.forEach((feedback) => {
      ...
      card.innerHTML = `
        <div class="public-feedback-top">
          <div class="public-feedback-name">${escapeHtml(feedback.name || t("anonymousUser"))}</div>
          <div class="public-feedback-date">${escapeHtml(formatFeedbackDate(feedback.created_at))}</div>
        </div>
        <div class="public-feedback-stars">${escapeHtml(getStars(feedback.rating))}</div>
        <div class="public-feedback-category">${escapeHtml(feedback.category)}</div>
        <p class="public-feedback-message">${escapeHtml(feedback.message)}</p>
      `;
      feedbacksList.appendChild(card);
    });
  } catch (error) {
    feedbacksList.innerHTML = `<div class="empty-state">${t("feedbacksLoadError")}</div>`;
  }
}
...
document.body.addEventListener("click", (event) => {
  ...
  const borrowId = target.dataset.borrow;
  const returnId = target.dataset.return;
...
if (loginHeroBtn) {
  loginHeroBtn.addEventListener("click", () => {
    if (state.user) showView("catalog");
    else openLogin();
  });
}
...
if (headerSearch)
  headerSearch.addEventListener(
    "keydown",
    (event) => event.key === "Enter" && runHeaderSearch(),
  );
if (closeLogin)
...
if (saveLogin) saveLogin.addEventListener("click", loginUser);
